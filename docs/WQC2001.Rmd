...
To obtain $\hat{F}(t)$ For a series of t, `sapply()` will do the trick. Also, theoritical value of $F(t)$ and $\Lambda(t)$ is computed. 
...
```{r}
curve(F.fun, 0, tau, col = 2)   # theoritical curve in red
...
Discrete Time (real world scenario)
-----------------------------------
