...
```{r loading-library}
library(recurrentR)
# All generators have been moved to a separated repo, RPPGen, which can be installed through Github
# library(devtools)
# install_github('RPPGen', 'wush978')
library(RPPGen)
library(plotrix)
```
...
```{r Lambda.hat1}
# ci: parameter c for asymptoic variance in paper WQC2001
c.hat.gen <- recurrentR:::c.hat.gen(obj)
ci <- sapply(1:length(t), c.hat.gen)
# >> SLOW STEP <<
# microbenchmark(sapply(1:length(t), c.hat.gen), times=5L)
# Unit: seconds
#                            expr      min      lq   median       uq      max neval
#  sapply(1:length(t), c.hat.gen) 2.622926 2.62529 2.693945 2.711106 2.740756     5
...
#            asymptotic.var 7.341220 8.835923 10.33063 11.347776 12.364927
# sapply(Lambda.hat.var(x)) 4.236351 5.431566  6.62678  8.015494  9.404209
...
for(i in 1:m) {
	...
	t.sim <- sapply(seq_along(y.sim), function(i) {
		# 	browser()
		zi <- gen_zi() * exp(as.vector(X[i,] %*% beta))
		lambda_i <- function(t) zi * lambda(t)
...
- due to auto dim-drop, `obj$Lambda.hat(T_0, bootstrap=TRUE)` will fail, should be explicitly in matrix form.
  - `apply(matrix(Lambda.hat.bootstrap.result, nrow=1), 1, stats::sd)`
...
for(i in 1:m) {
	...
	t.sim <- sapply(seq_along(y.sim), function(i) {
		# 	browser()
		z <- gen_zi() * exp(as.vector(X[i,] %*% beta))
		lambda_i <- function(t) z * lambda(t)
		retval <- gen_inhomo_poisson(lambda_i, y.sim[i] - 1, lambda_i(0))
		if (is.null(retval)) return(vector("numeric", 0))
		return(retval)
